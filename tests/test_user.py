...
def test_get_unexisted_user():
    ...
    response = client.get("/api/v1/user", params={'email': 'some@mail.com'})
    assert response.status_code == 404
    assert response.json() == {'detail': 'User not found'}

def test_create_user_with_valid_email():
    '''Создание пользователя с уникальной почтой'''
    new_user = {'name': 'Maksim', 'email': 'm.m.maksimov@mail.com'}
    response = client.post("/api/v1/user", json=new_user)
    assert response.status_code == 201
    assert isinstance(response.json(), int)
    
    created_user_response = client.get("/api/v1/user", params={'email': new_user['email']})
    assert created_user_response.status_code == 200
    assert created_user_response.json()['email'] == new_user['email']
    assert created_user_response.json()['name'] == new_user['name']
